...
for _, b_row in dfb.iterrows():
    ...
    # 1. Rolling 25m Block (Must be in 5m scan in last 25 mins)
    window_25 = sig_time - timedelta(minutes=25)
    in_5m = any(df5[(df5['symbol'] == sym) & (df5['date'] >= window_25) & (df5['date'] <= sig_time)])
    if not in_5m:
        final_results.append({"sym": sym, "t": sig_time.strftime("%H:%M"), "res": "REJECTED_MASTER_5M"})
        continue

    # 2. Flow Filter (Must have appeared in 1m and 3m scanners recently)
    # Usually within the last 5-10 minutes to show active momentum
    ...
    if not (in_1m and in_3m):
        final_results.append({"sym": sym, "t": sig_time.strftime("%H:%M"), "res": "REJECTED_FLOW_1M_3M"})
        continue

    # 3. Technical Filter (Dhan Data)
    data = get_data(sym)
    ...
    # 4. Simulation
    peak = entry_p; sl = entry_p * (1 - SL_PCT); tsl_on = False
    outcome = "EXIT_45M"; exit_p = data[min(len(data)-1, j+45)]['close']
